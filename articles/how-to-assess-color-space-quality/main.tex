...
\begin{figure}
  \centering
  \input{norms.tex}
  \caption{$p$-averages for the three sequences of length $n=20$, $s_\text{outlier} =
  \{1, 0,\dots, 0\}$, $s_\text{uniform} = \{0, h, \dots, 1\}$ (with $h = 1 / (n-1)$),
  and $s_\text{equal} = \{1, \dots, 1\}$.}
  \label{fig:1}
\end{figure}

% Each of the $\avg_p$ forms some sort of ``mean value''. Indeed, for $\rr=\{a,\dots,a\}$
% constant, $\avg_p(r) = |a|$ for all $p$.
%
% Which one should you choose? To answer this
% question, first note that $\avg_p(\rr) \le \avg_q(\rr)$ iff $p \le q$ for all $r$, so all
% $p$-averages are bound from below by the geometric mean and from above by the maximum.
%
% Consider the three sets of length $n$
% \[
%   \begin{split}
%     s_\text{outlier} &= \{1, 0,\dots, 0\},\\
%     s_\text{uniform} &= \{0, h, \dots, 1\} \text{ with } h = 1 / (n-1)\\
%     s_\text{equal} &= \{1, 1, \dots, 1\}.
%   \end{split}
% \]
% Clearly $\avg_p(s_\text{equal}) = 1$ for all $p$. Also $\avg_p(s_\text{outlier}) =
% n^{-1/p}$.
% \[
%   \avg_p(s_\text{uniform}) =
% \]
