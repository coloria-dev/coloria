...
\begin{remark}
  There are variants of $p_\text{STRESS}$, e.g.,
  \[
    \tilde{p}_\text{STRESS} = 100 \frac{\sqrt{\sum_{i=1}^n \frac{(\tilde{\alpha} d_i -
    \delta_i)^2}{\delta_i}}}{\sqrt{\sum_{i=1}^n \delta_i}}
    \quad\text{or}\quad
    \hat{p}_\text{STRESS} = 100 \sqrt{\sum_{i=1}^n \frac{(\hat{\alpha} d_i - \delta_i)^2}{\delta_i^2}}.
  \]
  which are large if any $\delta_i$ is small or vanishes.
  These can be useful in optimization when absolutely trying to prevent color distance
  or ellipse collapses.
\end{remark}
...
\begin{figure}
  ...
  \caption{Hung-Berns \cite{hung} hue linearity data for three color spaces.}
\end{figure}
