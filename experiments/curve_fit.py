...
def f(x, *p):
    return p[0]/numpy.sqrt(2*numpy.pi) * numpy.exp(-(x-p[1])**2 / (2*p[2]**2))


def f3(x, *p):
    return (
        + p[0]/numpy.sqrt(2*numpy.pi) * numpy.exp(-(x-p[1])**2 / (2*p[2]**2))
        + p[3]/numpy.sqrt(2*numpy.pi) * numpy.exp(-(x-p[4])**2 / (2*p[5]**2))
        + p[6]/numpy.sqrt(2*numpy.pi) * numpy.exp(-(x-p[7])**2 / (2*p[8]**2))
        )
...
def g(x, *p):
    return (
        p[0]
        * 1/numpy.sqrt(2*numpy.pi) * numpy.exp(-(x-p[1])**2 / (2*p[2]**2))
